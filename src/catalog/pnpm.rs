//! pnpm catalog detection (via `pnpm-workspace.yaml`).
...
impl CatalogProvider for PnpmCatalog {
        ...
        loop {
            ...
            if file.exists() {
                if let Ok(content) = std::fs::read_to_string(&file) {
                    let has_default =
                        content.contains("catalog:") && !content.contains("catalogs:");
                    ...
                    return Some(CatalogConfig {
                        file_path: file,
                        catalogs: vec![],
                        has_default_catalog: has_default || content.contains("\ncatalog:"),
